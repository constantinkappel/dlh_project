...
class CodeDataset(BaseDataset):
    ...
    def collator(self, samples):
        ...
        input = {"input_ids": torch.stack([s["input_ids"] for s in samples]).to(self.device)}
        input["seq_len"] = torch.stack([s["seq_len"] for s in samples]).to(self.device)
        input["value"] = torch.stack([s["value"] for s in samples]).to(self.device)
        out = {"label": torch.stack([s["label"] for s in samples]).to(self.device)}
...
class TokenizedDataset(BaseDataset):
    ...
    def collator(self, samples):
        ...
        input = {"input_ids": torch.stack([s["input_ids"] for s in samples]).to(self.device)}
        input["token_type_ids"] = torch.stack([s["token_type_ids"] for s in samples]).to(self.device)
        input["attention_mask"] = torch.stack([s["attention_mask"] for s in samples]).to(self.device)
        input["seq_len"] = torch.stack([s["seq_len"] for s in samples]).to(self.device)
        input["value"] = torch.stack([s["value"] for s in samples]).to(self.device)
        out = {"label": torch.stack([s["label"] for s in samples]).to(self.device)}
...
class MLMTokenizedDataset(BaseDataset):
    ...
    def collator(self, samples):
        ...
        input["token_type_ids"] = torch.stack([s["token_type_ids"] for s in samples]).to(self.device)
        input["attention_mask"] = torch.stack([s["attention_mask"] for s in samples]).to(self.device)
        out = {"label": torch.stack([s["mlm_labels"] for s in samples]).to(self.device)}
